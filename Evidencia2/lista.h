...
public:
  ...
  */
  ...
  /*Returns the index of the item to find*/
  int find(T data){
    if (!contains(data)) throw -1;
    Node<T> *temp = head;
    int index = 0;
    while (temp != NULL) {
      if (data == temp->data) {
  ...
  * Sobrecarga del operador [] para hacer un get
  */
  ...
  */
  ...
  friend ostream & operator << (ostream & os, LinkedList<T> & lista){
